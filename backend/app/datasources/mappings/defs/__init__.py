"""映射定义包：Python 注册（``fake`` / ``hithink`` / ``xuangutong`` / ``eltdx``）与 YAML 定义共存。
...
from app.datasources.mappings.defs import eltdx, fake, hithink, xuangutong

__all__ = ["eltdx", "fake", "hithink", "xuangutong"]
