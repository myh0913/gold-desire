...
- **幂等调度**：一次性任务当日成功不重跑；周期任务按间隔重跑（状态由
  :mod:`app.ingest.calendar_store` 落库）。
- **tick 循环**：可配置间隔，SIGTERM/SIGINT 优雅退出。
- 交易日历缓存、任务状态存储与每日维护拆至 :mod:`app.ingest.calendar_store`；
  ``is_trading_day`` / ``run_daily_maintenance`` 在此 re-export 保持兼容。
...
from app.ingest.calendar_store import (
    _MAINTENANCE_KEY,
    _POOL_AMOUNT_KEY,
    _now_sh,
    _state_mark,
...
from app.ingest.tasks import IngestTaskDef, all_tasks
