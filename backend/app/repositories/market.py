"""行情类仓储（``std_*`` 语义）：股票、日线、分时、情绪、快讯、主题、监管。

池/快照类仓储（涨停池、池快照、周期判定、天梯）见 :mod:`app.repositories.pool`，
此处 re-export 以保持既有 import 路径不变。一个聚合一个仓储，均继承
:class:`~app.repositories.base.BaseRepository`，对外暴露类型化查询方法，供服务层
与读 API（Task 12）调用。所有批量写入统一走 :meth:`BaseRepository.bulk_upsert`，
保证采集重跑的幂等性。
"""
...
from app.repositories.pool import (
